...
fields = ['frame', 'weight', 'intra_error', 'frame_avg_wavelet_energy', 'coded_error', 'sr_coded_error',
          'tr_coded_error', 'pcnt_inter',
          'pcnt_motion', 'pcnt_second_ref', 'pcnt_third_ref', 'pcnt_neutral', 'intra_skip_pct',
          'inactive_zone_rows',
...
def test_candidate_kf(dict_list, current_frame_index, frame_count_so_far):
    ...
    if ((qmode == False) or (frame_count_so_far > 2)) and (
        ...
        for i in range(0, 16):
            ...
            next_iiratio = (BOOST_FACTOR * lnf['intra_error'] /
...
def compose_aomsplit_first_pass_command(video_path: Path, stat_file: Path,
    ...
    video_params = re.sub(r'(--threads=[0-9]+)',
                          f'--threads={min(32, os.cpu_count() * 3)}',
                          video_params)
